...
namespace twg
{
	...
	//-------------------------------------------------------------------------
	/** Этот класс обеспечивает возможность создавать обработку сообщений окна в новом потоке. 

		Если вы не собираетесь создавать окна при помощи WinApi, то вам не обязательно знать про этот класс.

		Для создания окна необходимо наследоваться от этого класса и переопределить функцию `create`, в которой необходимо вернуть HWND созданного вами окна. Все сообщения этого окна будут поступать в виртуальную `wndProc`. Далее в конструкторе вашего окна необходимо вызвать onStart().

		Пример использования см. `window_object_use.cpp`.

		@note Для каждого окна создается независимая обработка сообщений окна! Если вы создали 3 окна, то у вас в программе будет 4 потока: 3 на сообщения окна, 1 текущий поток в функции `main()`. После закрытия определенного окна завершается и его поток.
	 */
	class WindowObject
	{
	public:
	...
	//-------------------------------------------------------------------------
	/** Класс для настройки внешнего вида окна. При создании объекта этого класса создается окно, параметры которого заданы в type. Можно изменять внешний вид окна при помощи функций-членов данного класса. */

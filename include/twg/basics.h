...
namespace twg
{
	...
	/** Останавливает выполнение программы, пока не пройдет время time. Время указано в миллисекундах. Т.е. если time = 1000, то время ожидания составит секунду. */
	void 	sleep(int32u time);
	/** Проверяет, нажата ли клавиша key. */
	bool 	isKeyDown(KeyType key);
	/** Возвращает время с начала старты системы в миллисекундах. Можно использовать для замера времени между промежутками. */
	int32u 	timeGet(void);
	/** Устанавливает курсор соответствующего типа. Тип курсоров смотри в \ref CursorType. */
	void 	setCursor(CursorType type);

	/** Преобразует нажатую клавишу key в строку. В строке находится такой символ, который бы сейчас напечатался в текстовом поле. Символ пишется с учетом текущей раскладки и нажатой клавиши Shift. */
	std::wstring key2wstr(KeyType key);

	/** Выводит маленькое окошко с текстом. */
	...
	enum CursorType : int32u
	{
		CURSOR_NULL, ///< Отсутствие курсора
		CURSOR_STANDART, ///< Обычный курсор
		CURSOR_CLICK, ///< Рука с вытянутым указательным пальцем. Такой курсор обычно ставится над ссылками или кнопками.
		CURSOR_WAIT, ///< Песочные часы. Обозначает ожидание, или что выполнение действия занимает время.
		CURSOR_STANDART_WAIT, ///<
		CURSOR_CROSS, ///< Четыре стрелки, направленные во все стороны света.
		CURSOR_HELP, ///< Помощь.
		CURSOR_TEXT, ///< Каретка. Обозначает, что в этом месте можно редактировать текст.
		CURSOR_FORBIDDEN, ///<
		CURSOR_MOVE, ///<
		CURSOR_UP, ///< Стрелка, направленная вверх.

		CURSOR_SIZE_UP_DOWN, ///< Вертикальный курсор, обозначающий, что сейчас ресайзится верхняя или нижняя часть окна.
		CURSOR_SIZE_LEFT_RIGHT, ///< Горизонтальный курсор, обозначающий, что сейчас ресайзится правая или левая сторона.
		CURSOR_SIZE_LEFT_DOWN_CORNER, ///< Диагональный курсор, обозначающий, что сейчас ресайзится левый нижний или правый верхний угол.
		CURSOR_SIZE_RIGHT_DOWN_CORNER ///< Диагональный курсор, обозначающий, что сейчас ресайзится левый верхний или правый нижний угол.
	};

	/** Тип всплывающего окошка. Первые параметры определяют какие кнопки будут в этом окне. Следующие параметры определяют иконку внутри окна. Параметры дальше определяют специфичные свойства. Параметры различных типов можно объеденять через оператор "побитовое или" - |. */
	enum MessageType : int32u
	{
		// Типы кнопок
		...
		/// Расположение текста
		MESSAGE_ALIGN_RIGHT_TEXT 	= 0x100, ///< Текст находится справа.
		MESSAGE_ALIGN_RIGHT_ALL		= 0x200, ///< И текст и кнопки находятся справа.

		/// Общие стили
		MESSAGE_STYLE_TOPMOST 		= 0x400 ///< Message Box находится поверх всех окон.
	};
	...
	/** Определяет код нажатой клавиши. Для того, чтобы узнать какому значению здесь соответствует клавиша на клавиатуре, запускай `window_events_test.cpp` и нажми эту клавишу. Там покажется ее название. */
	enum KeyType : int32u
	{
		KEY_A = 'A',
		KEY_B = 'B',

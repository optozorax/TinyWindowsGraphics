...
GIF_INLINE int GifIMax(int l, int r) { return l>r?l:r; }
GIF_INLINE int GifIMin(int l, int r) { return l<r?l:r; }
GIF_INLINE int GifIAbs(int i) { return i<0?-i:i; }

// walks the k-d tree to pick the palette entry for a desired color.
// Takes as in/out parameters the current best color and its error -
// only changes them if it finds a better color in its subtree.
// this is the major hotspot in the code at the moment.
GIF_INLINE void GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b, int& bestInd, int& bestDiff, int treeRoot = 1)
{
...
GIF_INLINE void GifSwapPixels(uint8_t* image, int pixA, int pixB)
{
...
GIF_INLINE int GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotIndex)
{
...
GIF_INLINE void GifPartitionByMedian(uint8_t* image, int left, int right, int com, int neededCenter)
{
    if(left < right-1)
    {
...
GIF_INLINE void GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, bool buildForDither, GifPalette* pal)
{
...
// changed pixels only.
GIF_INLINE int GifPickChangedPixels( const uint8_t* lastFrame, uint8_t* frame, int numPixels )
{
    int numChanged = 0;
...
GIF_INLINE void GifMakePalette( const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, GifPalette* pPal )
{
...
GIF_INLINE void GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    int numPixels = (int)(width * height);
...
GIF_INLINE void GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
...
// insert a single bit
GIF_INLINE void GifWriteBit( GifBitStatus& stat, uint32_t bit )
{
    bit = bit & 1;
...
GIF_INLINE void GifWriteChunk( FILE* f, GifBitStatus& stat )
{
...
GIF_INLINE void GifWriteCode( FILE* f, GifBitStatus& stat, uint32_t code, uint32_t length )
{
...
GIF_INLINE void GifWritePalette( const GifPalette* pPal, FILE* f )
{
...
GIF_INLINE void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
...
GIF_INLINE bool GifBegin( GifWriter* writer, const char* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth = 8, bool dither = false )
{
...
GIF_INLINE bool GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth = 8, bool dither = false )
{
    if(!writer->f) return false;
...
GIF_INLINE bool GifEnd( GifWriter* writer )
{
    if(!writer->f) return false;
